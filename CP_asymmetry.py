...
def compute_combined_calibration(data, plot: bool=True):
    ...
    counts, uncertainties, _ = dimuon_binning.B_counts(combined_peak_data, n_bins=1)
...
def asymmetry_calibrated(data, n_bins=10, plot: bool=False):
    ...
    for bin_counts, bin_unc in zip(counts, uncertainties):
        ...
        raw_asy, raw_unc = compute_b_asymmetry(B_plus, B_minus, B_p_unc, B_m_unc)
    ...
    if plot:
        ...
        ax2.axhline(0, color='red', linestyle='--', linewidth=1.5, label='SM Expectation ($A_{CP}=0$)')
        ax2.axhspan(-delta_A_unc, delta_A_unc, color='blue', alpha=0.1, label='Calibration Precision')
...
def rare_decay_asymmetry(data, n_bins=10, plot: bool=False):
    ...
    counts, uncertainties, inv_mass = dimuon_binning.B_counts(rare_data, n_bins)
    ...
    for i in range(len(counts)):
        ...
        if B_p_u <= 0 or B_m_u <= 0 or math.isnan(B_p_u) or math.isinf(B_p_u):
            print(f"Skipping bin {i} at {inv_mass[i]:.0f} MeV: Fit did not converge.")
            continue
    ...
    if plot:
        ...
        # Reference lines
        ax2.axhline(0, color='red', linestyle='--', alpha=0.6, label='SM Expectation ($A_{CP}=0$)')
        ax2.axhspan(-delta_A_unc, delta_A_unc, color='blue', alpha=0.1, label='Calibration Precision')
...
if __name__ == "__main__":
    ...
    acp_rare, acp_rare_unc, corrected_asy, mass_bins = rare_decay_asymmetry(signal_data, n_bins=4, plot=True)

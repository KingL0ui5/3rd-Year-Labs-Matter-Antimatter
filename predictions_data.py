...
def predict_all():
    ...
    for file in glob.glob('models/xgboost_model_*.pkl'):
        ...
        data_k = seperation.dataset_k(
            k+1, drop_cols=['B invariant mass', 'dimuon-system invariant mass', 'polarity'])
        ...
        pred_df = pd.DataFrame(predictions, columns=['signal_probability'], index=indexed_data_k.index)
        full_dataset = pd.merge(indexed_data_k, pred_df, left_index=True, right_index=True)
...
def background_fit_cleaning(data):
    ...
    data = data[(data['B invariant mass'] >= 5200) & (data['B invariant mass'] <= 6500)].reset_index(drop=True)

    data = data[(data['B invariant mass'] >= 5200) & (data['B invariant mass'] <= 6500)].reset_index(drop=True)
    hist, bin_edges = np.histogram(data['B invariant mass'], bins=200, range=(5200, 6500))
    bin_centers = (bin_edges[:-1] + bin_edges[1:]) / 2

    p0 = [5280, 20, 1.5, 2, np.max(hist), np.max(hist)/20, -0.0005, 1]
    try:
        popt, _ = curve_fit(total_fit_func, bin_centers, hist, p0=p0, 
    ...
    def get_bg_weight(mass):
        sig = crystal_ball(mass, *popt[0:5])
    ...
    ax1.hist(data['B invariant mass'], bins=100, alpha=0.3, label='Data', color='gray')
    ax1.plot(x_plot, total_fit_func(x_plot, *popt), 'r-', label='Total Fit (CB + Exp)')

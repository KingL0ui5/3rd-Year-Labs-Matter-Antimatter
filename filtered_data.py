...
class seperate:
    def __init__(self, k: int = None, plot: bool = False, dataset: str = '2011'):
        ...
        # 2. Load and label the external background (Same-sign)
        samesign = load_samesign()
        samesign['label'] = 0

        # 3. Initialize Fold Columns
        dataset['fold'] = -1
        # Still need folds for samesign to split TRAINING, even if not evaluating on it
        samesign['fold'] = -1
